/*  xorshift.h  –  device-side PRNG for OpenACC kernels
 *
 *  All functions are declared `#pragma acc routine seq` so they can be
 *  called from any gang / worker / vector context.
 *
 *  Compile with:  nvc++ -acc -gpu=cc80,managed -Minfo=accel ...
 */
#pragma once
#include <cstdint>
#include <cmath>          /* sqrt, log, cos  – nvc++ supports these on device */
