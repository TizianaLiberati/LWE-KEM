...
/*TODO: 
togli openfhe:
    - Elimina NativeVector, NativeInteger (in DiscreteUniformGeneratorImpl<NativeVector>, DiscreteGaussianGeneratorImpl<NativeVector>)
    - Hash (in SHA256 con SHAKE256)
togli Encaps/Decaps dal ciclo for
togli OpenMP
aggiungi FO con implicit rejection
rendi m=n e usa sempre matrici quadrate (usiamo Lattice a rango pieno)
*/  
...
// Funzione per generare una matrice casuale di dimensioni n x m con valori interi tra 0 e 30
...
// Posso parallelizzare rispetto j, non rispetto i perchè non sono indipendenti
...
void Encaps(uint32_t n, uint32_t m, uint32_t q, double stddev, std::vector<std::vector<int32_t>> &A, std::vector<int32_t> &t, std::vector<int32_t> &u, int32_t &v_i, int32_t plaintext_i, std::vector<int32_t> &Hash_K, std::vector<int32_t> &r, std::vector<int32_t> &e1, int32_t &e2)
{
    ...
    //auto startencaps = std::chrono::high_resolution_clock::now();
    HashUtil::Hash(conc_string, HashAlgorithm::SHA_256, digest_conc);
    //auto endencaps = std::chrono::high_resolution_clock::now();
    //auto GlobalTimeEnc = std::chrono::duration<double, std::milli>(endencaps - startencaps).count();
    //std::cout << "Global time hash " << GlobalTimeEnc << std::endl;
...
for (uint32_t i = 0; i < plaintext.size(); ++i)
{
    ...
    //auto endencaps = std::chrono::high_resolution_clock::now();
    //auto GlobalTimeEnc = std::chrono::duration<double, std::milli>(endencaps - startencaps).count();
    //std::cout << "Global time encapsuation " << GlobalTimeEnc << std::endl;
    //auto startdecaps = std::chrono::high_resolution_clock::now();
    Decaps(v_i, u, s, q, decrypt_i, t, n, m, stddev, A, r, e1, e2);
    //auto enddecaps = std::chrono::high_resolution_clock::now();
    //auto GlobalTimeDec = std::chrono::duration<double, std::milli>(enddecaps - startdecaps).count();
    //std::cout << "Global time decapsulation " <<GlobalTimeDec << std::endl;

    //auto endint = std::chrono::high_resolution_clock::now();
    //auto GlobalTimeInt = std::chrono::duration<double, std::milli>(endint - startint).count();
    //std::cout << "Global time interno " << GlobalTimeInt << std::endl;
